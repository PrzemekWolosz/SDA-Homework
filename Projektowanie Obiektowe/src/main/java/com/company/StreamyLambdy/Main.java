...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(mapOfCompaniesFromCity(companies));
    }
...
// Wszystkie rozwiązania zapisz w oddzielnych metodach statycznych w klasie Main.
    ...
    private static void allCompanies(List<Company> list) {
        System.out.println(list);
    }

    // 2. Wylistuj wszystkie firmy które są z Detroit
    private static void allCompaniesFromDetroit(List<Company> list) {
        list.stream()
                .filter(c -> c.getCityHeadquarters().equals("Detroit"))
                .forEach(System.out::println);
    }

    // 3. Wylistuj wszystkie firmy z Londynu, posortuj je po ilości pracowników (rosnąco).
    private static void allCompaniesFromLondonSortedByEmlpoyeesNumber(List<Company> list) {
        List<Company> newList = list.stream()
                .filter(c -> c.getCityHeadquarters().equals("London"))
                .collect(Collectors.toList());
        newList.sort(((company1, company2) -> Integer.compare(company1.getEmployees(), company2.getEmployees())));
        System.out.println(newList);
    }

    // 4. Wylistuj wszystkie firmy z Warszawy (nie ma ani jednej -> zamieniam na Gdansk zeby zrobic zadanie).
    // Posortuj je po ilości zakupów (rosnąco) i ilości pracowników (malejąco).
    private static void allCompaniesFromGdanskSortedByPurchaseAndEmployees(List<Company> list) {
        List<Company> newList = list.stream()
                .filter(c -> c.getCityHeadquarters().equals("Gdansk"))
                .collect(Collectors.toList());
        newList.sort((company1, company2) -> Integer.compare(company2.getEmployees(), company1.getEmployees()));
        //  newList.sort((company1, company2) -> Integer.compare(company1.getPurchaseList().size(), company2.getPurchaseList().size()));
        System.out.println(newList);
    }

    // 5. Zwróć firmę z największą ilością pracowników, która pochodzi z Kijowa.
    private static Company companyFromKijevWitchLargestNumberOfEmployees(List<Company> list) {
        List<Company> newList = list.stream()
                .filter(c -> c.getCityHeadquarters().equals("Kijev"))
                .collect(Collectors.toList());
        newList.sort((company1, company2) -> Integer.compare(company1.getEmployees(), company2.getEmployees()));
    ...
    private static Company companyWitchShortestName(List<Company> list) {
        list.sort((company1, company2) -> Integer.compare(company1.getName().length(), company2.getName().length()));
        return list.get(0);
    }

    // 7. Zwróć firmę która nie pochodzi z Kijowa, Londynu i Detroit, która ma najmniej kupionych produktów.
    private static Company companyNotFromKijevLondonDetroitWithLeastPurchase(List<Company> list) {
        List<Company> newList = list.stream()
                .filter(c -> !c.getCityHeadquarters().equals("Kijev"))
                .filter(c -> !c.getCityHeadquarters().equals("London"))
                .filter(c -> !c.getCityHeadquarters().equals("Detroit"))
    ...
    private static void addOneEmployeeIfCompanyIsFromKijevOrDetroit(List<Company> list) {
        List<Company> newlist1 = list.stream()
                .filter(c -> c.getCityHeadquarters().equals("Kijev"))
                .collect(Collectors.toList());
        List<Company> newList2 = list.stream()
                .filter(c -> c.getCityHeadquarters().equals("Detroit"))
                .collect(Collectors.toList());
        newlist1.addAll(newList2);
        for (Company c : newlist1) {
            c.setEmployees(c.getEmployees() + 1);
        }
    ...
    private static Map<String, Integer> companyKeyIsNameValueIsEmployeesNumber(List<Company> list) {
        Map<String, Integer> employeesNumber = new HashMap<>();
        for (Company c : list) {
            employeesNumber.put(c.getName(), c.getEmployees());
        }
    ...
    private static Map<String, List<Company>> mapOfCompaniesFromCity(List<Company> list) {
        Map<String, List<Company>> companiesList = new HashMap<>();
        for (Company c : list) {
            if (companiesList.containsKey(c.getCityHeadquarters())) {
                companiesList.get(c.getCityHeadquarters()).add(c);
            } else {
                companiesList.put(c.getCityHeadquarters(), new ArrayList<>());
                companiesList.get(c.getCityHeadquarters()).add(c);
            }
        }
    ...
    private static Company company_11_zakupy_najwieksze(List<Company> list) {
        Map<Double, Company> newPurchaseMap = new TreeMap<>();
        double maxPurchaseValue = 0;
        for (Company c : list) {
            double purchaseValue = 0;
            for (Purchase p : c.getPurchaseList()) {
                purchaseValue += p.getQuantity() * p.getProduct().getPrice();
            }
            newPurchaseMap.put(purchaseValue, c);
            if (purchaseValue > maxPurchaseValue) {
                maxPurchaseValue = purchaseValue;
            }
        }
        return newPurchaseMap.get(maxPurchaseValue);
    }

    // 12. Zwróć firmę która kupiła najwięcej produktów za kwotę wyższą niż 10 k
//    private static Company company_12_najwiecej_produktow_za_10k(List<Company> list) {
//        //        List<String> company = new ArrayList<>();
//        for (Company c : list) {
//            double purchaseValue = 0;
//            for (Purchase p : c.getPurchaseList()) {
//                purchaseValue += p.getProduct().getPrice() * p.getQuantity();
